"""
This module provides init packet request handler.
"""
...
class InitHandler(RequestPacketHandler):
    ...
    def handle(self, conn: ConnHandle, username: str, *args, **kwargs) -> InitPacket:
        """Validate params and produce key."""
        username_or_msg, success = self.validate_username(username)
        if not success:
            return InitPacket.response(
        ...
        assert key not in self.server.conns.keys(), "key already exists"
        conn.serve = True
        return InitPacket.response(
